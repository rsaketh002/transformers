...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_target, max_len, device, print_msg,  global_state, writer, num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_output= greedy_decode(model, encode_input, encoder_mask, tokenizer_src, tokenizer_target, max_len, device)
            
            source_text= batch['src_text'][0]
            target_text=batch['target_text'][0]
            
            model_out_text= tokenizer_target.decode(model_output.detach().cpu().numpy())
...
def get_ds(config):
    ...
    for item in ds_raw:
        ...
        target_ids=tokenizer_target.encode(item['translation'][config['lang_target']])
        max_len_src=max(max_len_src, len(src_ids))
        max_len_target=max(max_len_target,len(target_ids))
        
    print(f"Max length of the source sentence : {max_len_src}")
    print(f"Max length of the target sentence : {max_len_target}")
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            encoder_output=model.encode(encode_input, encoder_mask)
